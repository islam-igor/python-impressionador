...
Algumas empresas definem que um vendedor bateu a meta quando ele vendeu acima de X reais (outras ainda analisam não só as vendas desse vendedor
...
O ponto importante é, cada empresa tem alguma adaptação do cálculo de indicadores. E normalmente se formos analisar esses indicadores é 
 ter funções que façam todo o trabalho de análise deles. Por isso, vamos construir 2 exemplos aqui:

Item 1: Crie uma função que calcula o percentual de Stockout de uma empresa
O % de stockout é dado por (Vendas Perdidas por Estoque) / (Vendas Concluídas + Vendas Perdidas por Estoque) -> essas vendas são dadas em valor 
total (dinheiro) e não em quantidade de vendas
Seu programa recebe um dicionário com todas as vendas da empresa, o status dela (se foi Concluída ou Cancelada) e, caso tenha sido Cancelada, o
 motivo de Cancelamento. O formato é o seguinte:
...
def stockout(dicionario):   #islam
    soma_concluida = 0
    soma_stockout = 0
    for venda, status, motivo in dicionario.values():
        if status == 'Concluído':
            soma_concluida += venda
        elif motivo == 'Estoque em Falta':
            soma_stockout += venda
    return soma_stockout / (soma_concluida + soma_stockout)

'''Item 2: Crie uma função para descobrir os clientes inadimplentes de uma empresa
O objetivo é identificar quem são os clientes inadimplentes e enviar essa lista de clientes para o setor de cobrança poder
 fazer a cobrança dos clientes.
Sua função deve então receber uma lista de clientes, analisar quais clientes estão inadimplentes, e retornar uma lista com 
os clientes inadimplentes (apenas o CPF deles já é suficiente)
...
Isso significa que caso ou cliente esteja devendo 2.000 reais a 10 dias, ele não é inadimplente, pois não se passaram 20 
dias ainda. Da mesma forma, se ele estiver devendo 500 reais por 40 dias, ele também não é inadimplente, dado que ele deve
...
def cliente_inadiplente(lista):
    lista_inadiplente = []
    for cpf, divida, dias in lista:
        if divida > 1000 and dias > 20:
            lista_inadiplente.append(cpf)
    return lista_inadiplente

'''Exercícios
Antes de irmos para o desafio que apresentamos na última aula (que é bem mais complexo do que um exemplo simples) vamos
 resolver um exercício um pouco mais simples para treinar
1. Cálculo do Percentual e da Lista de Vendedores
Queremos criar uma function que consiga identificar os vendedores que bateram uma meta, mas além disso, consigo já me dar
 como resposta o cálculo do % da lista de vendedores que bateu a meta (para eu não precisar calcular manualmente depois)
Essa function deve receber 2 informações como parâmetro: a meta e um dicionário com os vendedores e suas vendas. E me dar 
2 respostas: uma lista com o nome dos vendedores que bateram a meta e o % de vendedores que bateu a meta.
'''
...
def bateu_meta(meta, vendas):
    vendedor_bateu_meta = []
    for vendedor, venda in vendas.items():
        if venda >= meta:
            vendedor_bateu_meta.append(vendedor)
    porcentagem = len(vendedor_bateu_meta) / len(vendas)
    return vendedor_bateu_meta, porcentagem


lista = bateu_meta(meta, lista_vendas)
print(lista)
