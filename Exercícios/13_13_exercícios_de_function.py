# -*- coding: utf-8 -*-
"""13.13 Exercícios de Function.ipynb

# Exercícios
## 1. Criando uma Lista de Produtos

Muitas vezes as listas de produtos, de clientes, de funcionários, etc que temos
nas empresas não são 100% confiáveis.
...
Vamos criar uma function que pega todas essas listas e cria, a partir dela, uma lista de
 produtos. Com o objetivo de saber todos os produtos que temos na empresa. Repare que a
 function pode receber 1, 2, 3 ou quantas listas de estoque forem necessárias.
...
def listar_produtos(*listas):
    ...
    lista_produtos = set(lista_produtos) #SET REMOVE AS DUPLICATAS , MAS NAO SE MANTEM COMO UMA LISTA, APENAS MOSTRA OS OBJETOS DISTINTOS
    lista_produtos = list(lista_produtos) #volta a ser uma lista
...
def dic_produtos(*listas):
    dic_produtos = {}
    for lista in listas:
        lista = dict(lista)
        for produto, estoque in lista.items():
            if not produto in dic_produtos:
                dic_produtos[produto] = estoque
            else:
                dic_produtos[produto] += estoque
...
x = dic_produtos(estoque_fim_jan, estoque_fim_fev, estoque_fim_mar, estoque_fim_abr)
